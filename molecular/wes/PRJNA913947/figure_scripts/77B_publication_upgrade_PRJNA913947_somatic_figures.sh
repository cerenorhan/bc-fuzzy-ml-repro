...
LOG="$LOGDIR/77B_publication_upgrade_PRJNA913947_somatic_figures_$(date +%Y%m%d_%H%M%S).log"
exec > >(tee -a "$LOG") 2>&1

echo "=== 77B: PUBLICATION UPGRADE - PRJNA913947 SOMATIC FIGURES ==="
date
...
PAIR_METRICS="$(find_first 'PRJNA913947_21pairs_somatic_PASS_pair_metrics.tsv')"
VAR_TYPE="$(find_first 'PRJNA913947_21pairs_somatic_PASS_variant_type_summary.tsv')"
...
echo "=== INPUTS FOUND ==="
printf "VAR_LONG\t%s\n" "${VAR_LONG:-MISSING}"
printf "PAIR_METRICS\t%s\n" "${PAIR_METRICS:-MISSING}"
printf "VAR_TYPE\t%s\n" "${VAR_TYPE:-MISSING}"
printf "FUNC_SUM\t%s\n" "${FUNC_SUM:-MISSING}"
printf "GENE_PAIR\t%s\n" "${GENE_PAIR:-MISSING}"
printf "ORA_AXIS\t%s\n" "${ORA_AXIS:-MISSING}"

if [[ -z "$VAR_LONG" && -z "$VAR_TYPE" ]]; then
  echo "ERROR: Variant long table or variant type summary is required."
  exit 1
fi

if [[ -z "$FUNC_SUM" ]]; then
  echo "ERROR: Functional gene summary table is required."
  exit 1
fi

export VAR_LONG PAIR_METRICS VAR_TYPE FUNC_SUM GENE_PAIR ORA_AXIS OUT STATUS
...
pair_metrics_file <- Sys.getenv("PAIR_METRICS")
var_type_file <- Sys.getenv("VAR_TYPE")
...
if (nzchar(var_type_file) && file.exists(var_type_file)) {
  vt <- read_tsv(var_type_file, show_col_types = FALSE)

  pair_col <- pick_col(vt, c("^pair$", "pair_id", "pair", "sample"), required = TRUE)
  class_col <- pick_col(vt, c("variant_type", "variant_class", "^type$", "class"), required = TRUE)
  count_col <- pick_col(vt, c("^n$", "n_variants", "count", "total"), required = FALSE)

  if (is.na(count_col)) {
    numeric_cols <- names(vt)[sapply(vt, is.numeric)]
    numeric_cols <- setdiff(numeric_cols, c(pair_col, class_col))
    if (length(numeric_cols) == 0) stop("No count column found in variant type table.")
  ...
  pair_class <- vt %>%
    transmute(
      ...
      variant_class = toupper(as.character(.data[[class_col]])),
      n = as.numeric(.data[[count_col]])
    )
} else {
  vl <- read_tsv(var_long_file, show_col_types = FALSE)

  pair_col <- pick_col(vl, c("^pair$", "pair_id", "pair", "sample"), required = TRUE)
  class_col <- pick_col(vl, c("variant_type", "variant_class", "^type$", "class"), required = TRUE)
...
pair_class <- pair_class %>%
  mutate(
    variant_class = case_when(
      str_detect(variant_class, "SNV|SNP") ~ "SNV",
      str_detect(variant_class, "INDEL|DEL|INS") ~ "INDEL",
      str_detect(variant_class, "MNP") ~ "MNP",
      TRUE ~ "OTHER"
    )
  ) %>%
  group_by(pair, variant_class) %>%
...
# Figure 1: stacked burden by pair
p_burden <- ggplot(pair_class, aes(x = pair_label, y = n, fill = variant_class)) +
  ...
  labs(
    title = "Somatic mutation burden across PRJNA913947 tumor-normal pairs",
...
# Figure 2: overall variant class composition
...
p_class <- ggplot(overall_class, aes(x = fct_reorder(variant_class, n), y = n, fill = variant_class)) +
...
p_genes <- ggplot(top_genes, aes(x = n_pairs, y = gene_label)) +
  ...
  labs(
    ...
    subtitle = "Point size indicates the number of functional variants; label indicates number of affected pairs",
...
if (nzchar(gene_pair_file) && file.exists(gene_pair_file)) {
  ...
  gp_pair_col <- pick_col(gp, c("^pair$", "pair_id", "pair", "sample"), required = TRUE)
  ...
  gp_plot <- expand_grid(gene = top_genes$gene, pair = pair_levels) %>%
    ...
    mutate(
      impact_class = ifelse(is.na(impact_class), NA_character_, impact_class),
  ...
  p_onco <- ggplot(gp_plot, aes(x = pair_label, y = gene)) +
    ...
    geom_tile(data = gp_plot %>% filter(!is.na(impact_class)),
...
if (nzchar(ora_axis_file) && file.exists(ora_axis_file)) {
  ...
  oa2 <- oa %>%
    ...
    filter(
      str_detect(reporting_tier, regex("FDR025|FDR_025|FDR≤0.25|FDR<=0.25|interpretable_FDR025", ignore_case = TRUE)) |
        (!is.na(best_padj) & best_padj <= 0.25)
    ) %>%
    mutate(
...
report <- file.path(statusdir, "77B_publication_upgrade_PRJNA913947_somatic_figures_status.txt")
sink(report)
cat("77B publication upgrade - PRJNA913947 somatic figures\n")
cat("Generated:", as.character(Sys.time()), "\n\n")

cat("Input files:\n")
cat("VAR_LONG:", var_long_file, "\n")
cat("VAR_TYPE:", var_type_file, "\n")
...
echo
echo "=== STATUS ==="
cat "$STATUS/77B_publication_upgrade_PRJNA913947_somatic_figures_status.txt" | head -140
