...
process_pair() {
    ...
    gatk --java-options "-Xmx24g -Djava.io.tmpdir=$TMP" MergeVcfs \
        "${MERGE_ARGS[@]}" \
        -O "$UNFILTERED"
    ...
    gatk --java-options "-Xmx8g -Djava.io.tmpdir=$TMP" LearnReadOrientationModel \
        "${F1R2_ARGS[@]}" \
        -O "$ORIENTATION"

    echo
    echo "=== GetPileupSummaries for $PAIR ==="
    gatk --java-options "-Xmx8g -Djava.io.tmpdir=$TMP" GetPileupSummaries \
        -I "$TUMOR_BAM" \
        -V "$GNOMAD_AF" \
        -L "$GNOMAD_AF" \
        -O "$TUMOR_PILEUPS"
    ...
    gatk --java-options "-Xmx8g -Djava.io.tmpdir=$TMP" CalculateContamination \
        -I "$TUMOR_PILEUPS" \
    ...
    gatk --java-options "-Xmx16g -Djava.io.tmpdir=$TMP" FilterMutectCalls \
        -R "$REF_FASTA" \
        -V "$UNFILTERED" \
        --stats "$MERGED_STATS" \
