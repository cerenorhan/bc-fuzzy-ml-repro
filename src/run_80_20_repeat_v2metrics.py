...
from .config import STAGE2_CENTERS, MAX_RULES
...
def run_one_split(X, Y, seed, test_size=0.20):
    ...
    for target_idx, target_name in enumerate(TARGETS):
        ...
        acc, wf1, mf1, bacc, C = eval_target(yte, yhat, centers_map[target_name])
        fuzzy_rows.append({
            "seed": seed, "target": target_name, "model": "fuzzy",
            "accuracy": acc, "weighted_f1": wf1, "macro_f1": mf1, "balanced_acc": bacc
        })

    # ---- ML baselines (including *_bal variants) ----
...
def main():
    ...
    print("Done. Wrote v2metrics repeat 80/20 outputs to outputs/")
    print("- metrics_test_fuzzy_80_20_repeats_v2metrics.csv (+ summary)")
    print("- metrics_test_ml_80_20_repeats_v2metrics.csv (+ summary)")
